...
let wrapperKey = 0;
const createWrapper = ({ userViewComponent }) => {
  return class Wrapper extends React.PureComponent {
    ...
    render() {
      if (this.state.hasError) {
        return (
          <View style={{ border: 'solid red 1px', padding: 10 }}>
...
class IdyllRuntime extends React.PureComponent {
  constructor(props) {
    ...
    const { vars, derived, data } = splitAST(getChildren(ast));
