...
// const parse = require('csv-parse/lib/es5/sync.js');
      ...
      if (typeof fetch !== 'undefined') {
        dataPromise = fetch(sourceValue)
          .then((res) => {
            if (res.status >= 400) {
              throw new Error(
                `Error Status ${res.status} occurred while fetching data from ${sourceValue}. If you are using a file to load the data and not a url, make sure async is not set to true.`,
              );
            }
...
export const findWrapTargets = (schema, state, components) => {
  ...
  mapTree(schema, (node) => {
    ...
    if (node.component) {
      const checkName = node.component.toLowerCase().split('-').join('');
